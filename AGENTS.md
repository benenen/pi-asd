...
`asd_list` / `asd_candidates` 只能找名字或确认 session 存在，不能拿它们的表面状态直接汇报。
必须接着对点名 session 调一次 `asd_peek`，读取屏幕里正在执行或已经执行完的具体内容，再据此
回答。peek 能读到多少就如实读多少；读不到就说明具体原因，再用本轮唯一一次 `asd_list` 确认
session 是否仍存在（已经查过就复用已有结果）。**只 peek 一次，不得借状态查询反复 peek 轮询。**
...
### 13. `asd_list` 只列全部 session 名字，绝不能拿来当 sleep
...
1. **用途收窄**：只有两个合法入口 —— 用户明确要看全部 asd session；或点名状态查询的
   单次 `asd_peek` 读不到，需要兜底确认 session 是否还存在。两个入口共享每轮一次额度，
   已经查过就复用已有结果。挑能接活的仍用 `asd_candidates`，看结果仍用 `asd_peek`；等待
   不占当前轮次，已有 watcher 就等推送，没有就调一次 `asd_follow` 注册后台监视
...
改提示词时仍然**一次都不能出现 `asd_agents`**；提 `asd_list` 时必须和「查全部 session / 点名
状态 peek 失败兜底」「两个入口共享每轮一次」「不是成败/等待信号」放在一起。有用例钉着这些边界。

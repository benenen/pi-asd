...
| `asd_agents` | 列出在监视的 agent、实时状态、以及各自屏幕最后一行 |
...
### asd_agents 回答的是「在不在动」，不是「干成了没有」

```
nvr [安静 3m10s persistent] (pi, /w/nvr): 长期负责 NVR 项目的排查
  屏幕最后一行：● 已把结论写进 Vikunja #1140 评论
pi-b [在动] (claude, /w/b): 查一下今天的新闻，写三条摘要
  ⚠️ 卡在对话框上等人作答（**不是失败，重发任务没用**）：❯ 1. Yes | 2. No
     用 asd_nav("pi-b", [...]) 作答
```

状态词是**「安静 / 在动」而不是 idle / running** —— 后者读起来像"闲着 = 没事干 =
做完了"，而它其实只是终端没动静：可能做完了、可能在沉默地想、也可能卡住了。

所以每行还会带上**屏幕最后一行**当凭据，卡在对话框上的单独标出来，列表底下也明写：

> "安静"只表示终端没动静，**不代表任务做成了**。要判断结果就 asd_peek 看屏幕、
> 或 asd_follow 等它真停下。**不要因为看着安静就重发任务。**

这条很重要：拿"安静"当失败信号去重发任务，只会让 agent 从头再来一遍，或者两份
任务同时在跑；而对卡在对话框上的那种，重发的文本会被对话框一起吃掉，完全无效。
...
| 情况 | 怎么判 | 通知 |
|---|---|---|
| 思考中 | settle 后隔 1.2s 再看一屏，**两屏不一样**说明还在重绘 | 不通知，安静重挂 |
| 等决策 | 两屏一致，且认出是模态对话框 | ⚠️ **需要用户决策** + 问题/选项/当前选中 + 怎么作答 |
| 真停下 | 两屏一致，不是对话框 | 已停下（历时 …）+ 最后一屏 |
...
指名交过任务的 session 会进监视列表，从此 `asd_agents` 一直追踪它。不想监视了：

```
asd_unmonitor("nvr")
```

**只是不管它了，不结束它。** 进程照跑，只是不再出现在 `asd_agents`、watcher 停掉、

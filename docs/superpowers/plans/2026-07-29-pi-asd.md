...
## Task 13: boss mode 提示词改成"flag 已开"的口吻，并告知工作区

**背景（两个必须一起修的不一致）：**

1. **提示词还写着"用户说 boss mode 时…"**，但注入现在由 flag 控制（`/asd:boss-start` /
   `PI_ASD_BOSS`）。flag 开着的时候这段提示词**已经在系统提示里了** —— 模型却读到"等用户
   说那个关键词再动"，于是可能一直不动。这是加开关时留下的自相矛盾：门已经开了，门里的
   牌子还写着"敲门后才开"。
2. **Task 12 之后 agent 拿到的是自己的空目录**，提示词却完全没提。模型不知道这件事，
   默认 spawn 出来的 agent 会在一个空目录里无事可干。提示词必须告诉它：活在哪个仓库
   就显式传 `cwd`。

顺带修准两处已经过时的描述：复用条件（Task 12 之后不给 `cwd` 时**不约束目录**）。

**Files:**
- Modify: `extensions/asd/prompt.ts`
- Test: `test/prompt.test.ts`

**Interfaces:** 不变（`bossModePrompt(input: PromptInput): string` 的签名和字段都不动）

- [ ] **Step 1: 给 `test/prompt.test.ts` 追加失败的测试**

```typescript
test("开启时的口吻是'已经开着'，不是'等用户说关键词'", () => {
  const p = bossModePrompt({ enabled: true, defaultAgent: "pi", agents: [] });
  assert.match(p, /已开启|已经开着/);
  // 关键：不能再让模型等一个触发词
  assert.doesNotMatch(p, /用户说 "boss mode" 时/);
  assert.doesNotMatch(p, /用户说“boss mode”时/);
});

test("提示词告诉模型新 agent 拿到的是空目录，要干仓库里的活得传 cwd", () => {
  const p = bossModePrompt({ enabled: true, defaultAgent: "pi", agents: [] });
  assert.match(p, /空目录/);
  assert.match(p, /cwd/);
});

test("提示词提醒别把小任务也拆开派出去", () => {
  const p = bossModePrompt({ enabled: true, defaultAgent: "pi", agents: [] });
  assert.match(p, /别 spawn|不要 spawn|自己做/);
});

test("复用的描述不再声称要求 cwd 相同", () => {
  const p = bossModePrompt({ enabled: true, defaultAgent: "pi", agents: [] });
  assert.doesNotMatch(p, /cwd 都对得上/);
});
```

**现有测试保持不变**——`/Boss Mode/`、`/asd_spawn/`、`/不要自己先调研|不要先自己调研/`、
`/不要等齐/`、`/不许 spawn 完就|必须跟到/`、`/asd_candidates/`、`/session:/`、
`/asd_kill 不会结束它/`、`/默认用 pi/`、以及 `enabled: false` 返回空串的三条。
**下面的新文案必须让它们全部继续通过**（关键词都保留了，只是重写了框架句）。

- [ ] **Step 2: 跑测试确认它失败**

Run: `npm test -- test/prompt.test.ts`
Expected: FAIL —— 现在的文案里有"用户说 "boss mode" 时"，也没有空目录/cwd 的说明

- [ ] **Step 3: 改 `extensions/asd/prompt.ts` 的 `definition` 数组**
...
```typescript
  const definition = [
    ...
    "## Boss Mode（已开启）",
    "",
    "boss mode 现在是开着的 —— 这就是你当前的工作方式，**不需要等用户说任何关键词**。",
    "",
    "遇到可以拆成互相独立子任务的活：立刻拆，用 asd_spawn 一个个派出去。",
    "不要自己先调研 —— 让派出去的 agent 并行去查，这才是 boss mode 的意义。",
    "每个子任务的描述必须自包含：带足上下文，让 agent 不用回来问也能干完。",
    "哪个先好就先处理哪个，不要等齐了再动。",
    "不许 spawn 完就回来向用户报告，必须跟到全部结束为止。",
    "",
    "**但也别滥用**：任务小到你自己几步就能做完、或者根本拆不出独立子任务时，就自己做，",
    "别 spawn。boss mode 是为真能并行的多子任务准备的，不是每件事都要派出去。",
    "",
    "### 派任务时必须想清楚目录",
    "",
    "asd_spawn 不显式给 cwd 时，新 agent 会拿到一个**属于它自己的空目录**（在工作区基坐",
    "目录下面，按 session 名分开），这样多个 agent 不会挤在同一个工作树上互相踩。",
    "代价是：那是个**空目录**，里面什么都没有。",
    "",
    "所以子任务如果是在某个已有仓库/目录里干活，**必须显式传 cwd 指过去**：",
    '  asd_spawn(task: "修 auth 的 bug", cwd: "/path/to/repo")',
    "只有当子任务本身就该在干净的空目录里做（比如从头搭一个新东西、或者任务里写明了",
    "自己去 clone），才可以不传 cwd。",
    "",
    "### 复用和收养",
    "",
    "asd_spawn 会先在台账里找 agent 对得上、而且已经空闲的 session 复用，找不到才新建 ——",
    "所以不用担心派活派出一堆重复 session。给了 cwd 时要求目录也精确相同；不给 cwd 时",
    "不约束目录，复用到的 agent 就在它自己的工作区里接新活。",
    `这一轮 spawn 默认用 ${input.defaultAgent}；要换就在 asd_spawn 里显式传 agent。`,
    "",
    "也可以把任务交给一个**已经在跑的**空闲 session（包括不是你 spawn 的）：先调",
    "asd_candidates 看它们各自在哪个目录、有什么项目文档、正在做什么，再用",
...
`bossSession` 那段追加逻辑**保持现状不动**（`if (input.bossSession !== undefined) definition.push(...)`）。

- [ ] **Step 4: 跑测试确认通过**

Run: `npm test -- test/prompt.test.ts`
Expected: PASS，新的 4 条 + 原有全部
...
```bash
git add extensions/asd/prompt.ts test/prompt.test.ts
git commit -m "fix: 提示词改成 flag 已开的口吻，并告知 agent 拿到的是空目录"
```

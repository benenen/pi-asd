...
| 工具 | 做什么 |
|---|---|
| `asd_spawn` | 派任务。先找空闲的复用，找不到才新建 session |
...
`asd_spawn` 不是每次都新开。台账里 agent 和 cwd 都对得上、而且当前空闲的 session
会被直接复用（把新任务 `asd send` 进去）。`reuse: false` 可以强制新建。

复用范围**只限本扩展 spawn 出来的 session**。原因是 `asd list --json` 不返回
session 的 cwd，判不出一个陌生 session 在哪个目录、跑的是不是同一种 agent。
...
- **只支持预设表里的 agent**（`pi` / `claude` / `codex`），不接受裸命令。
...
| follow watcher + `sendMessage(followUp, triggerTurn)` | Task 3、Task 7 |
| steer 后重挂 watcher | Task 5 `rewatch` |
| boss mode 提示词两段 + 禁 sleep 轮询 | Task 4 |
| shutdown 不杀 session、只掐 watcher、列存活 | Task 7 |
| 退出码 3 / 4 是状态不是错误 | Task 1 `follow` / `peek` / `send` / `kill` |
| 名字洗名 + 截断 + 避重 | Task 2 |
| e2e 同时设 `ASD_SOCKET` + `XDG_DATA_HOME` | Task 6 |
| 80x24 限制写进 README | Task 7 |

**Placeholder scan** — 无 TBD / TODO；每个代码步骤都有完整代码；每个测试步骤都有完整断言。

**Type consistency** — `Exec` / `ExecResult` / `SessionInfo` / `FollowOutcome`（Task 1）在 2/3/5/6 中按同名使用；`AgentRecord` 的七个字段（Task 2）在 5/6 构造时全部给齐；`WatcherPool` 的 `watch` / `stop` / `stopAll` / `isWatching` / `idle`（Task 3）在 5/6/7 中一致；`ToolResult` 的 `{ text, details?, isError? }`（Task 5）由 Task 7 的 `toolResult()` 统一翻译成 pi 的返回形状。
...
2. Task 7 的 `ctx.sessionManager.getHeader()` 返回类型可能不带 `parentSession`，需要一次窄化。**不要用 `as any`。**

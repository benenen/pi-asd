...
当前任务写进终端标题）、认出来的 agent 种类、闲了多久，以及**它是不是 pi-asd
自己建的**。

最后一项的判据必须是 `createdByUs === true`，**不是"在不在台账里"** —— 收养过
一次的 session 就在台账里了，但它永远杀不掉。用"在台账里"当判据会让这类 session
在第二次出现时不再显示"不能 kill"的提示，把 boss 误导到相反的结论上。
...
**第一段（恒定）**——沿用 pi-boss 那几条硬约束，但**口吻必须是"flag 已开"而不是
"等用户说关键词"**：提示词只在开关打开时注入，所以它出现在系统提示里这件事本身就是
触发条件。写成"用户说 boss mode 时…"会让模型在门已经开了的情况下继续等敲门。

- 立刻把任务拆成独立子任务并 spawn，**不要自己先调研**，让 spawn 出来的 agent 并行去查。
- 但也别滥用：拆不出独立子任务、或者自己几步就能做完时就自己做，不要 spawn。
- **必须告知工作区语义**：不给 `cwd` 的 spawn 拿到的是一个空目录，所以子任务如果在
  已有仓库里干活，必须显式传 `cwd`。不说这条，默认 spawn 出来的 agent 会在空目录里
  无事可干。

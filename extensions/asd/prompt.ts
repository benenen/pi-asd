...
export function bossModePrompt(input: PromptInput): string {
  ...
  const definition = [
    ...
    "4. 用户明确要看 **全部 asd session** 时，只调用一次 asd_list。它是 daemon 的会话清单，",
    "   包含不在 pi-asd 台账里的会话；只列名字，不读屏幕、不提供活动或任务成败，不能拿它轮询。",
    "",
  ...
  if (input.agents.length > 0) {
    ...
    sections.push(
      [
        ...
        "  不需要为此调用工具。asd_list 是用户明确要求时才查的全部 session 清单，不是监视工具。",

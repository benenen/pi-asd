...
export function bossModePrompt(input: PromptInput): string {
  ...
  const definition = [
    ...
    "4. asd_list 只有两种用途：用户明确要看 **全部 asd session**；或者点名状态查询的单次",
    "   asd_peek 读不到时兜底确认 session 是否存在。两种用途共享每轮最多一次的额度；它只列",
    "   名字，不读屏幕、不提供活动或任务成败，不能拿它轮询。",
    "5. **用户明确指定的状态汇报规则：** 用户问「<名字> 状态」「看下 <名字>」这类点名问题时，",
    "   asd_list / asd_candidates 只能用来找名字或确认 session 存在，不能单独完成状态汇报。",
    "   必须接着对这个名字调用一次 asd_peek，读取屏幕里正在执行或已经执行完的具体内容，再汇报。",
    "   peek 能读到多少就如实读多少；peek 读不到就说明具体原因，并改用一次 asd_list 确认 session",
    "   是否还存在（本轮已经查过就使用已有清单，不能再调第二次）。只 peek 一次，不要反复 peek 轮询。",
    "",
  ...
  if (input.agents.length > 0) {
    ...
    sections.push(
      [
        ...
        "  在那之前你可以去干别的、或者回复用户。",
        "- **上面这份清单每一轮都会重新算**，想知道派了什么、watcher 挂没挂，看它就够了，",
        "  不需要为此调用工具。asd_list 只在用户明确要看全部 session、或点名状态的 peek 失败时",
        "  兜底调用一次；它不是监视工具。",
        ...
        "- 上面台账内的 agent 若「卡在对话框上」，就用 asd_nav 作答。台账外 watcher 的通知会",
        "  指引你用 asd attach 手动处理。那不是失败，**重发任务对它没有任何",
        "  用处** —— 对话框会把你重发的文本一起吃掉。",
        "- 要监视某一个未挂 watcher 的 session，就调一次 asd_follow。它只注册后台监视并立即返回；",
        "  那个 session 停下来后，watcher 会自动把结果推给你。在此之前继续干别的，不要占住当前轮次。",
        "- 要给某个在跑的 agent 补充信息、或者纠偏，就调 asd_steer(session, message)。",
        "  **不要用 asd_spawn 再发一遍** —— 那是派一个新任务，不是补一句话。",
        "- **绝对不要轮询。** 不管用什么方式：bash sleep、反复 asd_peek、反复调任何一个",
        "  工具看有没有变化 —— 都算，工具是哪个不重要，**连着调**这个行为本身就是轮询。",
        "  等待不需要占住当前轮次：已有 watcher 就什么都不做；没有就调一次 asd_follow 注册后台监视。",
      ].join("\n"),
    );
  }
